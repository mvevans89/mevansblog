...
```{r page setup, echo = F, message = F, warning = F}
library(sp)
library(raster)
library(sf)
library(pryr)
...
```{r, echo = F, eval = F}
bike <- read.csv("data/2014-05 - Citi Bike trip data.csv")
```
...
results[2,3] <- system.time(st_as_sf(bike, 
                                     coords = c('start.station.longitude','start.station.latitude')))[2]
#get memory
...
<p align="center">
<img src="/img/computerExplore.gif" width="300px" >
</p>
...
For this round, the winner is `sp`.
...
```{r}
#get time
results[5,3] <- system.time(spTransform(route.sp, CRS("+init=epsg:2263")))[2]
results[6,3] <- system.time(st_transform(route.sf, "+init=epsg:2263"))[2]
...
```{r}
#get time
results[7,3] <- system.time(spPlot(spData = route.sp.NYCproj))[2]
results[8,3] <- system.time(sfPlot(sfData = route.sf.NYCproj))[2]
...
While it takes longer to create a simple feature from a csv, and the original object is larger, if you are going to be doing anything with that object, it's worth it. All the other tests (transforming, reproject, and plotting) were much faster when using `sf` methods than `sp`. They also used less memory overall.
